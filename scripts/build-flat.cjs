...
function makeStats(ts) {
  return { systemId: "pbta", systemVersion: "1.2.0", coreVersion: "14.366",
    createdTime: ts, modifiedTime: ts, lastModifiedBy: "the-sprawl-fr",
    compendiumSource: null, duplicateSource: null, exportSource: null };
}

function writeJSON(dir, data) {
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, data._id + ".json"), JSON.stringify(data, null, 2) + "\n", "utf8");
}
...
async function build() {
  ...
  console.log("\n1. Equipment...");
  const equipStore = {};
  const eqDir = path.join(base, "equipment");
  ...
  console.log("2. Moves...");
  const movesStore = {};
  const mvDir = path.join(base, "moves");
  ...
  const pbDir = path.join(base, "playbooks");
